...
For simple problems, multithreading might also make the solution slower, because it prevents the compiler from inlining the function and doing better optimizations, which tipically makes it at least 3x slower (sometimes orders of magnitude slower!). Therefore, in my setup, if the number of threads is set to 1, it will call the function directly rather than spawning and joining a single thread. In these scenarios where running single-threaded is better, I still try to write the solution in a way that is multithreaded capable, but I might leave some race conditions unfixed if they take too much time to pin down.
